...
#define SERV_TIME  2.00	    // Mean service time in seconds
...
int main(void)
{
  ofstream output;
  ofstream output3;
  output.open ("output_verify_debug_mmm.txt");
  output3.open ("output_verify_mmm.txt");
...
/*
  ...
  while (sim_time < end_time)
  {
	if (green)
	{
		if (t1 < t2)                	// *** Event #1 (arrival) ***
		{
			...
			output << "t1 = " << t1 << ", TL_green = " << TL_green << ", n = " << n \
			", next t1 = " << t1 ;
			if (n < 4)
			{	
		...
		else                        // *** Event #2 (departure) ***
		{
			...
			", n = " << n << ", C = " << c ;
			if (n > 2)
				t2 = sim_time + exponential(Ts/3);
			else
				t2 = SIM_TIME;
			output << ", next t2 " << t2 << endl;
			if (TL_green < (t2 - sim_time))
			{
	...
	{	//only arrival event occurs
		...
		{
			green = true;
			t2=sim_time+TL_red+exponential(Ts/3);
			n -= 2;
			TL_red = 60.00;
		}
	}
  ...
  // Compute outputs
  l = s / (sim_time-Tiddle);   // Compute mean number in system
